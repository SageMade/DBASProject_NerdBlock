...
{
    /// <summary>
    /// Taken from https://www.codeproject.com/tips/709121/move-and-resize-controls-on-a-form-at-runtime-with
    /// 
    /// Work on a better implementation (double buffer child controls or freeze rendering for instance)
    /// </summary>
    public class ControlMoverOrResizer
    {
        private const int EDGE_EPSILON = 8;
        ...
        private static void UpdateMouseEdgeProperties(Control control, Point mouseLocationInControl)
        {
            ...
            MouseIsInLeftEdge = Math.Abs(mouseLocationInControl.X) <= EDGE_EPSILON;
            MouseIsInRightEdge = Math.Abs(mouseLocationInControl.X - control.Width) <= EDGE_EPSILON;
            MouseIsInTopEdge = Math.Abs(mouseLocationInControl.Y) <= EDGE_EPSILON;
            MouseIsInBottomEdge = Math.Abs(mouseLocationInControl.Y - control.Height) <= EDGE_EPSILON;
        }
